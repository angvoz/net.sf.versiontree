...
public class DeepLayout extends AbstractLayout {
	...
	public void walk(ITreeElement treeElement, int x, int y) {
		...
		// for each revision...
		IRevision rev = AbstractVersionTreeHelper.getRevisionFromTreeElements(branch.getChildren());
		while (rev != null) {
			y++;
			// ...and if it has branches
			List<IBranch> branches = AbstractVersionTreeHelper.getBranchesForRevision(rev, emptyBranches, naBranches, branchFilter);
			if (!branches.isEmpty()) {
				int tmp_y = y;
				int i = 1;
				for (Iterator<IBranch> iterator = branches.iterator(); iterator.hasNext(); i++ ) {
					...
					tmp_y = Math.max( tmp_y,
						( (depth = reservedDepth.get( new Integer(x+i) )) != null
						 ? depth.intValue()+1 :0) );
					}
				y = tmp_y;
			}
			...
			// get next revision
			rev = AbstractVersionTreeHelper.getRevisionFromTreeElements(rev.getChildren());
		}
